...
```
backend/app/{api,policy,ratelimit,executors,events,models,core,services}
...
**Phases 1 (Setup), 2 (Core Data Model), 3 (Policy Engine) and 4 (Rate Limiting) are done.** Backend and frontend both scaffold, install, and run cleanly.
...
- **Seed:** `python -m app.seed` (idempotent) creates `demo-email-agent`, `demo-payments-agent`, 4 rules (global send_email→allow, global make_payment→deny, payments-agent make_payment→needs_approval priority 10, payments-agent make_payment with `amount` lte 5000 → allow priority 20), and rate limits (default send_email 10/60s and make_payment 5/60s; payments-agent make_payment 3/60s + 20/day).
- **Policy engine (Phase 3):** `backend/app/policy/`. `conditions.py`: rule condition format `{"all": [{"field": "amount", "op": "lte", "value": 5000}]}` (`{}` = always matches; `field` is a dotted payload path), ops `eq, neq, lt, lte, gt, gte, in, not_in, email_domain_in, email_domain_not_in`, one function per op, validated by the Pydantic `RuleConditions` model. `engine.py`: pure `evaluate(AgentContext, ActionContext, rules) -> Decision(effect, code, reason, rule_id)`, using frozen snapshots built via `AgentContext.from_model` / `Rule.from_model`, never ORM objects. Precedence (ADR-005): inactive agent or unregistered action type → DENY; then the first matching rule by priority desc, then agent-specific over global, then most restrictive; an unevaluable condition → DENY `policy_error`; no match → DENY `no_matching_rule`. Payment `amount` is an integer in cents. Loading rules from the DB and writing the decision event happen in the decision-flow service (below).
- **Rate limiting (Phase 4):** `RateLimit` model (`rate_limits`: `agent_id` NULL = default, `action_type`, `max_requests`, `window_seconds`, `is_active`). If an agent has any limits of its own for an action type, they replace the defaults; several windows can apply together. `backend/app/ratelimit/limiter.py`: `RateLimiter.hit(agent_id, action_type, limits)` runs a Redis sliding-window log in one atomic Lua script. It is all-or-nothing across windows, rejected requests aren't counted, and it takes an injectable `clock` for tests. Keys look like `ratelimit:{agent_id}:{action_type}:{window_seconds}`. `select_limits()` is pure.
- **Decision flow:** `backend/app/services/firewall.py::submit_action_request(session, limiter, agent_id, action_type, payload)`: load the agent (unknown agent → `AgentNotFoundError`, nothing recorded), load and snapshot the rules (malformed rule → DENY `policy_error`), `evaluate`, then, if not denied, run the rate limiter (over the limit → DENY `rate_limited`; `RedisError` → DENY `rate_limiter_unavailable`). It then inserts the `ActionRequest` plus exactly one decision event via `app/events/store.py::append_event` (actor `firewall`, `data` holds `code`, `rule_id` and rate-limit details) and commits. **No HTTP route exposes this yet.**
- **Tests:** `pytest` from `backend/` (`pytest.ini` sets asyncio auto mode). Tests run against the migrated dev DB inside a rolled-back transaction, and against Redis using a per-test key prefix that is deleted afterwards (the `limiter` fixture uses `FakeClock`). Requires `docker compose up -d` first.
...
## Next step: Phase 5 (Approval Workflow)
- Open decision: how agents authenticate to `POST /actions/request` (e.g. per-agent API keys, stored hashed). Right now anyone who knows an agent UUID could act as that agent, so settle this before exposing the route.
- Thin `POST /actions/request` route that calls `submit_action_request` (404 for an unknown agent).
- Approval queue: pending = requests whose latest event is `needs_approval` (derived from `audit_log`, no status column). Endpoints to list pending, approve and reject; the approver must be authenticated (JWT); each approval or rejection writes an event with actor `approver:<username>`; a request that's already resolved can't be approved again.
- Approval dashboard UI, plus the end-to-end test: request → needs_approval → human decision → (executed later, in Phase 6) or blocked.
